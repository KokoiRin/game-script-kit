...
- **THEN** UI 日志展示对应的关键中文事件
- **AND** UI 不展示普通步骤开始和步骤成功噪音

#### Scenario: 展示未满足前置条件

- **WHEN** 后台脚本因为等待条件超时或图片目标未找到而正常停止
- **THEN** UI 的黑色运行输出框展示停止原因
- **AND** UI 不把该情况展示为运行失败

#### Scenario: 兼容文本图片匹配日志

- **WHEN** 脚本状态 payload 没有结构化事件但 stdout 包含图片匹配日志
- **THEN** UI 日志使用中文展示该次图片匹配的模板路径、耗时毫秒、是否命中和置信度摘要
